...
try
{
	...
	$statement = $db->prepare('SELECT id, book, chapter, verse, content FROM scripturesOC');
	$statement->execute();
	...
	while ($row = $statement->fetch(PDO::FETCH_ASSOC))
	{
		...
		$stmtTopics = $db->prepare('SELECT name FROM topicOC t'
			. ' INNER JOIN scripture_topicOC st ON st.topicId = t.id'
